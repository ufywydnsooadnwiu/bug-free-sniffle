...
class BotCommandAgent(BaseAgent):
    ...
    def initialize_agent(self):
        ...
        # A list of cars for both teammates and opponents sorted based on the distance to the ball
        self.team_closest_ball = []
        self.opp_closest_ball = []
        # Usefull objects
    ...
    def update_vars(self):
        # Make a deepcopy of both self.friends and self.foes
        self.team_closest_ball = deepcopy(self.friends)
        self.opp_closest_ball = deepcopy(self.foes)
        # Include our car in the sorting process
        self.team_closest_ball.append(self.me)
        # Sort based on the ball distance
        self.team_closest_ball = sorted(self.team_closest_ball, key=lambda x: x.location.dist(self.ball.location))
        self.opp_closest_ball = sorted(self.opp_closest_ball, key=lambda x: x.location.dist(self.ball.location))
    ...
    def debug_intent(self):
        ...
        text_intent = self.get_intent().__class__.__name__
        text_state = self.state.__class__.__name__
        self.renderer.draw_string_2d(10, 100, 3, 3, text_intent, white)
        self.renderer.draw_string_2d(10, 250, 3, 3, text_state, white)
...
class car_object:
    ...
    def update(self, packet):
        ...
        self.av.data = [car.physics.angular_velocity.x,
                              car.physics.angular_velocity.y, car.physics.angular_velocity.z]
        ...
        self.time_on_ground = self.time_on_ground + 1/120 if not self.airborne else 0
        self.time_in_air = self.time_in_air + 1/120 if self.airborne else 0
...
class ball_object:
    ...
    def update(self, packet):
        ...
        self.angular_velocity.data = [ball.physics.angular_velocity.x, ball.physics.angular_velocity.y, ball.physics.angular_velocity.z]
        self.latest_touched_time = ball.latest_touch.time_seconds
        self.latest_touched_team = ball.latest_touch.team

    def predict(self, agent: BotCommandAgent, t):
        ball_prediction = agent.get_ball_prediction_struct()
        start_time = ball_prediction.slices[0].game_seconds
        approx_index = int((agent.time + t - start_time) * 60)  # We know that there are 60 slices per second.
        if 0 <= approx_index < ball_prediction.num_slices:
            phys = ball_prediction.slices[approx_index].physics
            return (Vector3(phys.location), Vector3(phys.velocity), Vector3(phys.angular_velocity))
        return (self.location, self.velocity, self.angular_velocity)
...
class game_object:
    ...
    def update(self, packet):
        ...
        self.time_after_kickoff = self.time_after_kickoff + 1/120 if not self.kickoff else 0
